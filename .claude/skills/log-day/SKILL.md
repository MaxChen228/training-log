...
### type=`meal`（**規劃中，目前 app UI 還沒有，但 jsonb 不需 schema migration 即可寫**）
```jsonc
{
  ...通用,
  "type": "meal",
  ...
  "items": [
    {
      "name": "牛肉麵",
      "qty": "1 碗",
      "kcal": 650, "p": 32, "f": 22, "c": 78,
      "source": "claude_estimate" | "tfnd:<id>" | "usda:<fdcId>" | "off:<barcode>" | "manual"
    }
  ],
  "photo_url": null,
  "ai_inferred": true              // 是否來自 Claude Vision 估算
}
```

### type=`recovery`（**規劃中，同上**）
```jsonc
{
  ...通用,
  "type": "recovery",
...
- **photo_url（meal block）**：若有圖，先上傳到 Supabase Storage（bucket 待建立 `meal-photos`），存回傳的 public URL；目前 bucket 還沒建，先設 null 並提醒使用者
